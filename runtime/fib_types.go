...
typedef unsigned char fib_bool;
typedef unsigned char fib_uchar;
...
#ifdef FIB_ENABLE_ZVAL_LONG64
# ifdef FIB_WIN32
#  define FIB_SIZE_MAX  _UI64_MAX
# else
#  define FIB_SIZE_MAX  SIZE_MAX
# endif
#else
# if defined(FIB_WIN32)
#  define FIB_SIZE_MAX  _UI32_MAX
# else
#  define FIB_SIZE_MAX SIZE_MAX
# endif
#endif

typedef intptr_t fib_intptr_t;
typedef uintptr_t fib_uintptr_t;

#ifdef ZTS
#define FIB_TLS static TSRM_TLS
#define FIB_EXT_TLS TSRM_TLS
#else
#define FIB_TLS static
#define FIB_EXT_TLS
#endif

typedef struct _fib_object_handlers fib_object_handlers;
typedef struct _fib_class_entry     fib_class_entry;
typedef union  _fib_function        fib_function;
typedef struct _fib_execute_data    fib_execute_data;

typedef struct _zval_struct     zval;

typedef struct _fib_refcounted fib_refcounted;
typedef struct _fib_string     fib_string;
typedef struct _fib_array      fib_array;
typedef struct _fib_object     fib_object;
typedef struct _fib_resource   fib_resource;
typedef struct _fib_reference  fib_reference;
typedef struct _fib_ast_ref    fib_ast_ref;
typedef struct _fib_ast        fib_ast;
...
/*
 * fib_type - is an abstraction layer to represent information about type hint.
 * It shouldn't be used directly. Only through FIB_TYPE_* macros.
 *
 * FIB_TYPE_IS_SET()     - checks if type-hint exists
 * FIB_TYPE_IS_CODE()    - checks if type-hint refer to standard type
 * FIB_TYPE_IS_CLASS()   - checks if type-hint refer to some class
 *
 * FIB_TYPE_NAME()       - returns referenced class name
 * FIB_TYPE_CE()         - returns referenced class entry
 * FIB_TYPE_CODE()       - returns standard type code (e.g. IS_LONG, _IS_BOOL)
 *
 * FIB_TYPE_ALLOW_NULL() - checks if NULL is allowed
 *
 * FIB_TYPE_ENCODE() and FIB_TYPE_ENCODE_CLASS() should be used for
 * construction.
 */

typedef uintptr_t fib_type;

#define FIB_TYPE_IS_SET(t) \
	((t) > Z_L(1))

#define FIB_TYPE_IS_CODE(t) \
	(((t) > Z_L(1)) && ((t) <= Z_L(0x1ff)))

#define FIB_TYPE_IS_CLASS(t) \
	((t) > Z_L(0x1ff))

#define FIB_TYPE_NAME(t) \
	((fib_string*)((t) & ~Z_L(0x3)))

#define FIB_TYPE_CE(t) \
	((fib_class_entry*)((t) & ~Z_L(0x3)))

#define FIB_TYPE_CODE(t) \
	((t) >> Z_L(1))

#define FIB_TYPE_ALLOW_NULL(t) \
	(((t) & Z_L(0x1)) != 0)

#define FIB_TYPE_ENCODE(code, allow_null) \
	(((code) << Z_L(1)) | ((allow_null) ? Z_L(0x1) : Z_L(0x0)))

#define FIB_TYPE_ENCODE_CLASS(class_name, allow_null) \
	(((uintptr_t)(class_name)) | ((allow_null) ? Z_L(0x1) : Z_L(0)))

#define FIB_TYPE_ENCODE_CLASS_CONST_0(class_name) \
	((fib_type) class_name)
#define FIB_TYPE_ENCODE_CLASS_CONST_1(class_name) \
	((fib_type) "?" class_name)
#define FIB_TYPE_ENCODE_CLASS_CONST_Q2(macro, class_name) \
	macro(class_name)
#define FIB_TYPE_ENCODE_CLASS_CONST_Q1(allow_null, class_name) \
	FIB_TYPE_ENCODE_CLASS_CONST_Q2(FIB_TYPE_ENCODE_CLASS_CONST_ ##allow_null, class_name)
#define FIB_TYPE_ENCODE_CLASS_CONST(class_name, allow_null) \
	FIB_TYPE_ENCODE_CLASS_CONST_Q1(allow_null, class_name)
...
#define FIB_SAME_FAKE_TYPE(faketype, realtype) ( \
	(faketype) == (realtype) \
...
#define ZVAL_NEW_ARR(z) do {									\
		zval *__z = (z);										\
		fib_array *_arr =										\
		(fib_array *) emalloc(sizeof(fib_array));				\
		Z_ARR_P(__z) = _arr;									\
		Z_TYPE_INFO_P(__z) = IS_ARRAY_EX;						\
	} while (0)
...
#define ZVAL_NEW_PERSISTENT_ARR(z) do {							\
		zval *__z = (z);										\
		fib_array *_arr =										\
		(fib_array *) malloc(sizeof(fib_array));				\
		Z_ARR_P(__z) = _arr;									\
		Z_TYPE_INFO_P(__z) = IS_ARRAY_EX;						\
	} while (0)
...
#define ZVAL_NEW_RES(z, h, p, t) do {							\
		fib_resource *_res =									\
		(fib_resource *) emalloc(sizeof(fib_resource));		\
		zval *__z;												\
		GC_REFCOUNT(_res) = 1;									\
...
#define ZVAL_NEW_PERSISTENT_RES(z, h, p, t) do {				\
		fib_resource *_res =									\
		(fib_resource *) malloc(sizeof(fib_resource));		\
		zval *__z;												\
		GC_REFCOUNT(_res) = 1;									\
		GC_TYPE_INFO(_res) = IS_RESOURCE;						\
		_res->handle = (h);										\
...
#define ZVAL_NEW_EMPTY_REF(z) do {								\
		fib_reference *_ref =									\
		(fib_reference *) emalloc(sizeof(fib_reference));		\
		GC_REFCOUNT(_ref) = 1;									\
...
#define ZVAL_NEW_PERSISTENT_REF(z, r) do {						\
		fib_reference *_ref =									\
		(fib_reference *) malloc(sizeof(fib_reference));		\
		GC_REFCOUNT(_ref) = 1;									\
		GC_TYPE_INFO(_ref) = IS_REFERENCE;						\
		ZVAL_COPY_VALUE(&_ref->val, r);							\
		Z_REF_P(z) = _ref;										\
...
#define ZVAL_NEW_AST(z, a) do {									\
		zval *__z = (z);										\
		fib_ast_ref *_ast =									\
		(fib_ast_ref *) emalloc(sizeof(fib_ast_ref));			\
		GC_REFCOUNT(_ast) = 1;									\
...
#define ZVAL_COPY_VALUE(z, v)							\
	do {												\
		zval *_z1 = (z);								\
		const zval *_z2 = (v);							\
		fib_refcounted *_gc = Z_COUNTED_P(_z2);		\
	...
	do {												\
		...
		if ((_t & ((IS_TYPE_REFCOUNTED|IS_TYPE_COPYABLE) << Z_TYPE_FLAGS_SHIFT)) != 0) { \
			if ((_t & (IS_TYPE_COPYABLE << Z_TYPE_FLAGS_SHIFT)) != 0) { \
				_zval_copy_ctor_func(_z1 FIB_FILE_LINE_CC); \
			} else {									\
				GC_REFCOUNT(_gc)++;						\
			}											\
		}												\
...
#define ZVAL_UNREF(z) do {								\
		zval *_z = (z);									\
		fib_reference *ref;							\
		FIB_ASSERT(Z_ISREF_P(_z));						\
		ref = Z_REF_P(_z);								\
		ZVAL_COPY_VALUE(_z, &ref->val);					\
		efree_size(ref, sizeof(fib_reference));		\
	} while (0)
...
#define SEPARATE_ARRAY(zv) do {							\
		zval *_zv = (zv);								\
		fib_array *_arr = Z_ARR_P(_zv);				\
		if (UNEXPECTED(GC_REFCOUNT(_arr) > 1)) {		\
			if (Z_REFCOUNTED_P(_zv)) {					\
				GC_REFCOUNT(_arr)--;					\
			}											\
			ZVAL_ARR(_zv, fib_array_dup(_arr));		\
		}												\
	} while (0)

#define SEPARATE_ZVAL_NOREF(zv) do {					\
		zval *_zv = (zv);								\
		FIB_ASSERT(Z_TYPE_P(_zv) != IS_REFERENCE);		\
		if (Z_COPYABLE_P(_zv)) {						\
			if (Z_REFCOUNT_P(_zv) > 1) {				\
				if (Z_REFCOUNTED_P(_zv)) {				\
					Z_DELREF_P(_zv);					\
